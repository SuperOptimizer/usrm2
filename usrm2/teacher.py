...
def run(out, z0, y0, x0, Z, Y, X, volume=data.CT, window=256, halo=32, tile=1536, ckpt=CKPT, device=None):
    """Tiles over y/x (each tile read with a `halo` margin, blended, then cropped) so RAM stays bounded."""
    ...
    for y in range(0, Y, tile):
        for x in range(0, X, tile):
            ya, yb, xa, xb = max(y - halo, 0), min(y + tile + halo, Y), max(x - halo, 0), min(x + tile + halo, X)
