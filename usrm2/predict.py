...
def predict(ckpt, volume, z0, y0, x0, Z, Y, X, out, window=128, halo=16, device=None, volcomp=True):
    ...
    net.eval()
    ct = data.open_zarr(volume)
    roi = ct[z0:z0 + Z, y0:y0 + Y, x0:x0 + X]
    acc, wsum, g = np.zeros((Z, Y, X), np.float32), np.zeros((Z, Y, X), np.float32), gauss(window)
    ...
    prob = np.where(wsum > 0, acc / np.maximum(wsum, 1e-6), 0)
    a = out_array(out, (Z, Y, X), (z0, y0, x0), volcomp=volcomp)
