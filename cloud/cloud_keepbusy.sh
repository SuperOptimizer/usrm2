...
while true; do
  for h in tnr-0 tnr-1; do
    if ! ssh -o ConnectTimeout=20 $h "pgrep -f 'cloud/run_boxes.s[h]' > /dev/null" 2>/dev/null; then
      ...
      if [ -f ~/cloud_last_$h ]; then  # did the seed that just finished here come up short? then queue it for backfill
        L=$(cat ~/cloud_last_$h); c=$(ssh $h "grep '^box ' out/boxes$L.log 2>/dev/null | sort -u | wc -l")
        [ "${c:-0}" -lt 200 ] && { echo "$(date) $h seed $L ended with $c/200 boxes -> backfill"; echo $L >> ~/cloud_backfill; }
      fi
