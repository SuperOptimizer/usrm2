#!/bin/bash
# On the A100: the big-patch multi-resolution 4-head run. usage: SIZE=26m CKPT=-1 ADDSKIP=1 PATCH="352 352 352" bash cloud/a100_train.sh RUN
# Reads only local data (~/teacher stores, the local CT mirror at /vesuvius/usrm/volcomp incl. pyramid levels).
R=${1:-r4_a100}; SIZE=${SIZE:-26m}; CKPT=${CKPT:--1}; ADDSKIP=${ADDSKIP:-1}; PATCH=${PATCH:-352 352 352}; STEPS=${STEPS:-40000}
